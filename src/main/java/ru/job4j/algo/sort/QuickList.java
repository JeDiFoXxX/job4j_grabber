...
public class QuickList {
    ...
    private static <T> void quickSort(List<T> sequence, int start, int end, Comparator<T> comparator) {
        if (start < end) {
            int supportPoint = breakPartition(sequence, start, end, comparator);
            quickSort(sequence, start, supportPoint - 1, comparator);
            quickSort(sequence, supportPoint + 1, end, comparator);
        }
    }

    private static <T> int breakPartition(List<T> sequence, int start, int end, Comparator<T> comparator) {
        T supportPoint = sequence.get(start);
        int beginToEnd = start + 1;
        int endToBegin = end;
        while (true) {
            while (beginToEnd <= end && comparator.compare(sequence.get(beginToEnd), supportPoint) < 0) {
                beginToEnd++;
            }
            while (endToBegin >= start && comparator.compare(sequence.get(endToBegin), supportPoint) > 0) {
                endToBegin--;
            }
